    ...
    def update(self):
        if self.update_tf_data():
            ...
            if abs(self.distance) < 0.1:
                ...
                self.cmd_vel.angular.z = 0.1 * self.path_angle_err
...
class Turn90Degrees(py_trees.behaviour.Behaviour):
    ...
    def update(self):
        ...
        if elapsed_time < 2.0:  # Turn for 2 seconds to achieve roughly 90 degrees
            self.cmd_vel.angular.z = pi / 2 / 2.0  # Set a rotation speed for 90 degrees in 2 seconds
            self.cmd_vel.linear.x = 0.0
            self.draw_square.move_cmd.publish(self.cmd_vel)
            rospy.loginfo(f"Turning: angular.z={self.cmd_vel.angular.z}")
            return py_trees.common.Status.RUNNING
        else:
            self.stop_robot()
            return py_trees.common.Status.SUCCESS
